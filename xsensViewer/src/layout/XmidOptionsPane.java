...
public class XmidOptionsPane extends BorderPane {
	...
	public static void main(String[] args) {
		...
		System.out.println("----Sensor test----");

		int[] rawMessage = new int[] {250, 255, 193, 16, 32, 48, 0, 25, 52, 39, 255, 255, 160, 81, 0, 200, 107, 100, 255, 255, 232};

		XBusMessage xsensMessage  = XSensMessage.XbusMessage_getXBusMessage(rawMessage); 

		System.out.println("XMID: XMID_OutputConfig: " +  xsensMessage.len);

		ArrayList<XsDataIdentifier> outputConfig = XSensMessage.get_OutputConfig(xsensMessage.charBufferRx); 
		XMIDOptions xMIDOptions2 = new XMIDOptions(); 

		for (int i=0; i<outputConfig.size(); i++) {
			if (outputConfig.get(i).getFrequency()!=null && outputConfig.get(i).getFrequency()>0) {
				System.out.println("Angle output: "  +  outputConfig.get(i) + " sR: " + outputConfig.get(i).getFrequency()); 
				xMIDOptions2.angleOuput = outputConfig.get(i); 
				xMIDOptions2.sampleRate = outputConfig.get(i).getFrequency(); 
			}
		}
		...
		int[] rawMessage2 = new int[] {250, 255, 73, 4, 0, 0, 0, 0, 180};

		XBusMessage xsensMessage2  = XSensMessage.XbusMessage_getXBusMessage(rawMessage2); 

		ArrayList<XsOptionID> optionsFlags = XSensMessage.get_OptionConfig(xsensMessage2.charBufferRx); 
		
		System.out.println("XMID: XMID_SetOption: " +  optionsFlags.size());
		for (int i=0; i<optionsFlags.size(); i++) {
			System.out.println("Options output: "  +  optionsFlags.get(i)); 
		}
